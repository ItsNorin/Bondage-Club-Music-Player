// ==UserScript==
// @name         BC Music Player
// @namespace    BC-MusicPlayer
// @version      0.0.1
// @description  Music Player for that Bondage Club Game
// @author       ItsNorin
...
// @grant        none
// ==/UserScript==
const BC_Music_Version = "0.0.1";
const BC_Music_FullName = "Bondage Club Music Player";

var BC_MusicSendJoinMsg = true;
const BC_MusicFirstJoinMsg = BC_Music_FullName + ' v' + BC_Music_Version + ' loaded. Use <b>/music</b> for help.';
...
const MusicPlayer = (function () {
    var _musicQ = new Array();

    function _isValidMusicUrl(url) {
        return typeof url === "string"
            && (url.length <= 250)
            && CommonURLHasExtension(url, [".mp3", ".mp4"]);
    }

    function _setCustomMusicInfo(musicUrl) {
        var newRoom = ChatRoomData;

        if (newRoom.Custom == null) {
            newRoom.Custom = {};
        }
        newRoom.Custom.MusicURL = musicUrl;
    ...
    return {
        ...
        // attempts to set chat room music to given URL
        // returns error text to be shown to user
        setRoomMusic(url) {
            if (!ChatRoomPlayerIsAdmin()) {
                return "Must be room admin to modify music";
            }
            if (!_isValidMusicUrl(url)) {
                return "Invalid music url, must be a .mp3 or .mp4 and under 250 characters";
            }
            _playNewMusic(url);
...
(async function () {
    ...
    if (Commands.some((a) => a.Tag === BC_Music_Command.Tag)) {
        console.log('Unable to register /music ');
    }
    else {
